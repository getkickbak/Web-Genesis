...
__init__ = function()
{
   ...
   KickBak.fn =
   {
      ...
      convertDateCommon : function(v, dateFormat, noConvert)
      {
         ...
         if (!( v instanceof Date))
         {
            if ( typeof (JSON) != 'undefined')
            {
               ...
               //v = (KickBak.fn.isNative()) ? v : v.split('.')[0];
            }
      ...
      {
         return Math.floor(((em / fontsize) * KickBak.constants.defaultFontSize));
      },
      calcPxEm : function(px, em, fontsize)
      {
         return ((px / KickBak.constants.defaultFontSize / fontsize) + (em / fontsize));
      },
      ...
      processSendLocalID : function(result, cancelFn)
      {
         ...
         if (result.freqs)
         {
            KickBak.constants.lastLocalID = result.freqs;
         }

         localID = KickBak.constants.lastLocalID;
         if (localID)
         {
   ...
   // Ext.dom.Element
   // **************************************************************************
   Ext.define('KickBak.dom.Element',
   {
   ...
   // Ext.util.Collection
   // **************************************************************************
   Ext.define('KickBak.util.Collection',
   {
   ...
   // **************************************************************************
   Ext.define('KickBak.data.reader.Json',
   {
   ...
   Ext.define('KickBak.data.writer.Writer',
   {
   ...
   // **************************************************************************
   Ext.define('KickBak.data.proxy.Server',
   {
         ...
         switch (metaData['rescode'])
         {
            ...
            case 'login_invalid_facebook_info' :
            {
               Ext.device.Notification.show(
               {
                  ...
                  message : KickBak.constants.createAccountMsg,
                  buttons : ['OK'],
                  callback : function(btn)
                  {
      ...
      processResponse : function(success, operation, request, response, callback, scope)
      {
         ...
         {
            messages = ((resultSet && Ext.isDefined(resultSet.getMessage)) ? (Ext.isArray(resultSet.getMessage()) ? resultSet.getMessage().join(KickBak.constants.addCRLF()) : resultSet.getMessage()) : 'Error Connecting to Server');
         ...
         if (!response.aborted)
         {
            ...
            //if ((success === true) || (KickBak.fn.isNative() === true))
            if (success === true)
            {
   ...
   // Ext.data.Connection
   // **************************************************************************
   Ext.define('KickBak.data.Connection',
   {
      ...
      parseStatus : function(status, xhr)
      {
         ...
         /*
          ...
          if (KickBak.fn.isNative() && (status === 0))
          {
          success = true;
          }
          */
   ...
   // Ext.field.Select
   // **************************************************************************
   Ext.define('KickBak.field.Select',
   {
    ...
    * @private
    */
   Ext.define('KickBak.dataview.element.List',
   {
   ...
   // **************************************************************************
   Ext.define('KickBak.device.connection.PhoneGap',
   {
   ...
   Ext.define('Ext.device.notification.Abstract',
   {
       ...
       * The buttons to be displayed on the notification. It can be a string, which is the title of the button, or an array of
       * multiple
       * strings.
   ...
   // **************************************************************************
   Ext.define('KickBak.util.Geolocation',
   {
   ...
   // **************************************************************************
   Ext.define('KickBak.data.proxy.PagingMemory',
   {
      extend : 'Ext.data.proxy.Memory',
      alias : 'proxy.pagingmemory',
      alternateClassName : 'KickBak.data.PagingMemoryProxy',
      /**
   ...
   // **************************************************************************
   Ext.define('KickBak.plugin.ListPaging',
   {
   ...
   // **************************************************************************
   Ext.define('KickBak.plugin.PullRefresh',
   {
