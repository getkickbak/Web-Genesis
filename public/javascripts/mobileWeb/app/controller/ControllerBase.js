Ext.define('KickBak.controller.ControllerBase',
{
   ...
   geoLocationErrorMsg : function()
   {
      ...
      if (Ext.os.is('Android'))
      {
         ...
         'Enable Location Services under Main Screen of your phone: \"SignUp App >> Location Services >> GPS satellites\"';
      }
      else if (Ext.os.is('iOS'))
      {
         rc += ((Ext.os.version.isLessThan('6.0')) ? //
         'Enable Location Services under Main Screen of your phone: \"SignUp App >> Location Services >> KICKBAK\"' :
         // //
         'Enable Location Services under Main Screen of your phone: \"SignUp App >> Privacy >> Location Services >> KICKBAK\"'//
         );
      }
      else
      {
         rc += 'Enable Location Services under Main Screen of your phone: \"SignUp App >> Location Services\"';
      }
   ...
   missingVenueInfoMsg : function(errors)
   {
      ...
      {
         errorMsg = KickBak.constants.addCRLF() + errors;
      }
      else if (Ext.isObject(errors))
      {
         errorMsg = KickBak.constants.addCRLF() + errors.statusText;
      }
   ...
   {
      return 'Have you been referred ' + KickBak.constants.addCRLF() + //
      'by a friend to visit' + KickBak.constants.addCRLF() + //
      merchatName + '?';
   },
   recvReferralb4VisitMsg : function(name)
   {
      return 'Claim your reward points by becoming a customer at ' + KickBak.constants.addCRLF() + name + '!';
   },
   ...
   inheritableStatics :
   {
      ...
      playSoundFile : function(sound_file, successCallback, failCallback)
      {
         ...
         {
            sound_file['successCallback'] = successCallback || Ext.emptyFn;
            var sound = Ext.get(sound_file['name']);
            if (sound)
            {
               sound.dom.play();
            }
      ...
      encryptFromParams : function(params, mode)
      {
         ...
         var encrypted = null, venueId = KickBak.fn.getPrivKey('venueId'), key = null;
         if ((venueId > 0) || (venueId < 0))
         {
            try
            {
               switch (mode)
               {
                  case 'prize' :
                  {
                     key = KickBak.fn.getPrivKey('p' + venueId);
                  }
                  case 'reward' :
                  {
                     key = KickBak.fn.getPrivKey('r' + venueId);
                     break;
                  }
                  default :
                     key = KickBak.fn.getPrivKey('r' + venueId);
      ...
      genQRCodeFromParams : function(params, mode, encryptOnly)
      {
         ...
         GibberishAES.size(256);
         var venueId = KickBak.fn.getPrivKey('venueId');
         var key = "";
         switch (mode)
         {
            case 'prize' :
   ...
   checkReferralPrompt : function(cbOnSuccess, cbOnFail)
   {
      ...
      && (!KickBak.db.getReferralDBAttrib("m" + merchantId)))// Haven't been referred by a friend yet
      {
         ...
         me.earnRedeemPopup = (Ext.create('Ext.Sheet',
            {
               ...
               items : [
               {
                  ...
                  html : me.fbConnectRequestMsg + '<img width="160" style="margin:0.7em 0;" src="' + KickBak.constants.resourceSite + 'images/' + 'facebook_icon.png"/>'
               },
               {
                  docked : 'bottom',
                  defaults :
   ...
   accelerometerHandler : function(vol, callback)
   {
      ...
      navigator.accelerometer.getCurrentAcceleration(function(accel)
      {
         ...
         {
            if (vol != KickBak.constants.s_vol)
            {
               window.plugins.proximityID.setVolume(KickBak.constants.s_vol);
               console.debug('Accelerometer new_vol=' + KickBak.constants.s_vol);
               callback(KickBak.constants.s_vol);
            }
         }
         else
         {
   ...
   {
      var me = this, c = KickBak.constants, viewport = me.getViewPortCntlr();
      ...
      var scan = function()
      {
         ...
         window.plugins.proximityID.scan(function(result)
         {
            ...
            var identifiers = KickBak.fn.processRecvLocalID(result);
   ...
   {
      var me = this, c = KickBak.constants;
      me.send_vol = -1;
      ...
      window.plugins.proximityID.send(function(result)
      {
         ...
         success(KickBak.fn.processSendLocalID(result, cancel));
      }, function(error)
      {
         console.log('Error Code[' + Ext.encode(error) + ']');
         cancel();
   ...
   persistStore : function(storeName)
   {
      ...
      for (i in stores)
      {
         if (!stores[i][0])
         {
            ...
            {
               model : 'KickBak.model.' + stores[i][2],
         ...
         if (!storeName || (stores[i][1] == storeName))
         {
            ...
            if (!cleanOnly)
            {
               ...
               for ( x = 0; x < items.length; x++)
               {
                  ...
                  stores[i][0].add(Ext.create('KickBak.model.CustomerJSON',
                  {
                     json : json
                  }));
               }
