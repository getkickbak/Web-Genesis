Ext.define('Ext.plugin.GMap.Tracker',
{
   ...
   constructor : function(config)
   {
      ...
      {
         this.provider = this.provider || (navigator.geolocation ? navigator.geolocation : (window.google ||
         {
         }).gears ? google.gears.factory.create('beta.geolocation') : null);
      }

      /**
       * @event beforeupdate
       * @param {Coordinates} coord A coordinate object as defined by the coords property. Will return false if geolocation is
       * disabled or denied access.
       * @param {Ext.util.GeoLocation} this
       */

      /**
       * @event update
       * @param {Cooridinates} coord A coordinate object as defined by the coords property. Will return false if geolocation is
       * disabled or denied access.
       * @param {Ext.util.GeoLocation} this
       */
   ...
   startUpdates : function()
   {
      ...
      {
         this.watchId = this.provider.watchPosition(this.onPosition, this.onError,
         {
   ...
   updateLocation : function(callback, scope)
   {
      ...
      if(Ext.supports.GeoLocation)
      {
         ...
         me.provider.getCurrentPosition(function(position)
         {
   ...
   parseCoords : function(location, asObject)
   {
      ...
      if(coords && asObject !== false && (window.google ||
      {
      }).maps)
      {
