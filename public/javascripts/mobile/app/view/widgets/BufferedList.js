...
Ext.define('Ext.ux.view.widgets.BufferedList',
{
   ...
   {
      var me = this, item = e.getTarget().parentNode, index = me.recordIndexFromNode(item), record = me.getStore().getAt(index);
   ...
   doRefresh : function()
   {
      ...
      if (!this.firstRefreshDone)
      {
         ...
         this.topProxy = Ext.Element.create(
         {
            cls : 'ux-top-proxy'
         });
         this.listContainer = Ext.Element.create(
         {
            cls : 'ux-list-container'
         });
         this.bottomProxy = Ext.Element.create(
         {
            cls : 'ux-bottom-proxy'
         });
         ...
         {
            this.refreshItemListAt(0);
            // renders first this.getMinimumItems() nodes in store
         }
   ...
   renderOnScroll : function()
   {
      ...
      var startIdx, ds = this.getStore(), scrollPos = this.scroller.position.y, newTop = null, newBottom = null, previousTop = this.topItemRendered, previousBottom = this.bottomItemRendered, scrollDown = scrollPos >= this.lastScrollPos, incrementalRender = false, maxIndex = this.getRecordCount() - 1, thisHeight = this.element.dom.clientHeight, listHeight = this.listContainer.getHeight(), topProxyHeight = this.topProxy.getHeight();
      ...
      if (newTop === null || newBottom === null || (incrementalRender && newTop >= previousTop && newBottom <= previousBottom
      ))
      {
   ...
   updateListHeader : function(scrollPos)
   {
      ...
      var i, headerNode, headerHeight = this.header.renderElement.getHeight(), nHeaders = this.groupHeaders.length, headerMoveTop = scrollPos + headerHeight, groupTop, transform, headerText;
   ...
   buildItemHtml : function(firstItem, lastItem, groupHeads)
   {
      ...
      var configArray = [], store = this.getStore(), grpHeads = this.getGrouped(), selected = this.getSelection();
   ...
   {
      var insert = mode === 'i', append = mode === 'a', replace = mode === 'r', topProxyHeight = 0, sc = this.getRecordCount(), oldListHeight = this.listContainer.getHeight(), groupHeads = [], firstNode, lastNew;
   ...
   createGroupingMap : function()
   {
      ...
      var store = this.getStore(), prevGroup = '', sc = store.getCount(), i;
      ...
      if (!!this.getIndexBar())
      {
         ...
         var l = 0, letters = this.getIndexBar().getLetters(), bc = letters.length, key;
   ...
   isItemRendered : function(index)
   {
      ...
      return this.getViewItems().length > 0 ? index >= this.topItemRendered && index <= this.bottomItemRendered : false;
   },
   ...
   {
      var startPos = this.scroller.position.y, elems = this.getViewItems(), nElems = elems.length, returnArray = [], thisHeight = this.element.dom.clientHeight, firstItem = this.topItemRendered;
   ...
   {
      var me = this, index = me.indexOfRecord(record), item = me.nodeFromRecordIndex(index);
      // SMB patch
   ...
   {
      var me = this, target = e.getTarget(), index = me.recordIndexFromNode(target), // SMB patch
      store = me.getStore(), record = this.getRecordAt(index), pressedDelay = me.getPressedDelay(), item = Ext.get(target);
   ...
   {
      var me = this, target = e.getTarget(), index = me.recordIndexFromNode(target), // SMB patch
      store = me.getStore(), record = this.getRecordAt(index), item = Ext.get(target);
   ...
   {
      var me = this, target = e.getTarget(), index = me.recordIndexFromNode(target), // SMB patch
      item = Ext.get(target), record = this.getRecordAt(index);
