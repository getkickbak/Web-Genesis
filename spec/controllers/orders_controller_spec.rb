...
describe OrdersController do
  ...
  describe "GET show" do
    it "assigns the requested order as @order" do
      Order.stub(:get).with("37") { mock_order }
      get :show, :id => "37"
  ...
  describe "PUT update" do
    ...
    describe "with valid params" do
      it "updates the requested order" do
        Order.should_receive(:get).with("37") { mock_order }
        mock_order.should_receive(:update).with({'these' => 'params'})
        put :update, :id => "37", :order => {'these' => 'params'}
      end

      it "assigns the requested order as @order" do
        Order.stub(:get) { mock_order(:update => true) }
        put :update, :id => "1"
        assigns(:order).should be(mock_order)
      end
    ...
    describe "with invalid params" do
      it "assigns the order as @order" do
        Order.stub(:get) { mock_order(:update => false) }
        put :update, :id => "1"
        assigns(:order).should be(mock_order)
      end
