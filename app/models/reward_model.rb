...
class RewardModel
  ...
  attr_accessible :type_id, :signup_amount, :signup_points, :rebate_rate, :prize_rebate_rate, :price_per_point, :price_per_prize_point, :expected_avg_spend
  ...
  validates_presence_of :expected_avg_spend, :if => lambda { |t| !t.type_id.nil? && RewardModelType.id_to_value[t.type_id] == "amount_spend"  }
  validates_with_method :signup_amount, :method => :check_signup_amount, :if => lambda { |t| !t.type_id.nil? && RewardModelType.id_to_value[t.type_id] == "amount_spend"  }
  validates_with_method :expected_avg_spend, :method => :check_expected_avg_spend, :if => lambda { |t| !t.type_id.nil? && RewardModelType.id_to_value[t.type_id] == "amount_spend"  }
  ...
  def update(type, reward_model_info)
    ...
    self.expected_avg_spend = reward_model_info[:expected_avg_spend]
    self.update_ts = now
    self.type = type
    save
  end
