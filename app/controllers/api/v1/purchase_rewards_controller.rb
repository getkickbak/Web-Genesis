  ...
  def earn_common
    ...
    begin
      Customer.transaction do
        @customer_mutex = CacheMutex.new(@customer.cache_mutex_key, Cache.memcache)
        ...
        if challenge && (referral_record = ReferralChallengeRecord.first(:referral_id => @customer.id, :status => :pending)) && (referral_record.referrer_id != @customer.id)
          ...
          @referrer_mutex = CacheMutex.new(referrer.cache_mutex_key, Cache.memcache)
        ...
        @venue_mutex = CacheMutex.new(@venue.cache_mutex_key, Cache.memcache)
